...
public class roomServiceLogin {
    ...
    public static JLabel roomServiceImg;
    public static establishConnection dc = new establishConnection();

    public roomServiceLogin(){
        widgetDesign design = new widgetDesign();
        homeFrame =design.createFrame("ROOM SERVICE|LOGIN",75, 20, 1500, 800);
        ...
        roomServiceImg =design.setImageAsBackground("images/roomServiceLogo.png",690,220,150,150);
        homeFrame.add(roomServiceImg);
    ...
    private static class Action_performed implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource()==login){
                try{
                    String q="select * from employee_detail where designation='Cleaner' and emp_id='"+userField.getText()+"' and pass ='"+passField.getText()+"'";
                    ResultSet res=dc.sta.executeQuery(q);
                    if (res.next()){
                        JOptionPane.showMessageDialog(null,"Login Successfully");
                        new roomServiceDashBoard();
